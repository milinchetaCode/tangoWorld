...
export class ApplicationsService {
    ...
    async apply(userId: string, eventId: string) {
        ...
        const acceptedMale = event.applications.filter(a => a.user.gender === 'male').length;
        const acceptedFemale = event.applications.filter(a => a.user.gender === 'female').length;
        ...
        } else if (user.gender === 'female' && acceptedFemale >= event.femaleCapacity) {
            initialStatus = 'waitlisted';
